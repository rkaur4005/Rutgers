...
(function(angular) {
    ...
    angular
        ...
        .controller('AssessmentCrtCtrl', ['$scope', '$window', '$routeParams', '$http', '$uibModal', '$filter', 'globalConfig', 'Auth', 'assessmentfields', 'NavItems','$location', function($scope, $window, $routeParams, $http, $uibModal, $filter, globalConfig, Auth, assessmentfields, NavItems, $location) {
            ...
            $scope.$watch('formdata.open_date', function(newval, oldval){
                if($scope.formdata.close_date < $scope.formdata.open_date) {
                    $scope.formdata.close_date = '';
                }
            });

            $scope.$watch('formdata.close_date', function(newval, oldval){
                if($scope.formdata.close_date < $scope.formdata.open_date) {
                    $scope.formdata.close_date = '';
                }
            });
            ...
            //console.log($scope.formdata.assessmentsList);
            $scope.formdata = {
                classsections: [{
                    assessment_id: null,
        ...
        .controller('AssessmentMCrtCtrl', ['$scope', '$window', '$routeParams', '$http', '$uibModal', '$filter', 'globalConfig', 'Auth', 'assessmentfields','$location' ,'NavItems',   function($scope, $window, $routeParams, $http, $uibModal, $filter, globalConfig, Auth, assessmentfields,$location, NavItems) {
            $scope.assessmentshown = "create";
            NavItems.navitems.nav = {};
            assessmentfields.setclassid($routeParams.classId);
            // toggle two tabs create Assessment Add Segment
        ...
        .controller('AssessmentMReviewCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
            $scope.user_type = Auth.getUserType();
            $scope.user_id = $routeParams.user_id;
            $scope.uuid = $routeParams.uuid;
            var params = {
            ...
            $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.mreview + '?' + $.param(params))
                .then(function (data) {
                    if (data.data) {
                        angular.forEach(data.data.mastery.questions, function (qanda) {
                            qanda.question_text = $sce.trustAsHtml(qanda.question_text);
                            angular.forEach(qanda.answers, function (ans) {
                                ans.formatted = $sce.trustAsHtml(ans.formatted);
                            });
                        ...
                        $scope.mastery = data.data.mastery;
                        angular.forEach($scope.mastery.questions, function (qanda, i) {
                            $timeout(function () {
                                MathJax.Hub.Queue(["Typeset", MathJax.Hub, 'question' + i]);
                            }, 1000);
                        ...
                        NavItems.errmsg = 'Error getting Mastery Assessment Review';
                    }
                }, function() {
                    NavItems.errmsg = 'Error getting Mastery Assessment Review';
                });
        ...
        .controller('AssessmentListCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
            ...
            $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.listclass + '?' + $.param(params))
                .then(function (data) {
        ...
        .controller('AssessmentUserLookCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
            $scope.assessment_id = $routeParams.assessmentId;
            $scope.netid = '';
            $scope.loadassessmentreport = function () {
                var params;
                params = {
                    callback: 'JSON_CALLBACK',
                    methodtype: 'GET',
                ...
                $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.assessmenthistory + '?' + $.param(params))
                    .then(function (data) {
                        if (data.data) {
                            $scope.assessments = data.data.assessments;
                            $scope.error = '';
                        } else {
                            NavItems.errmsg = 'Error getting User Data';
                        }
                    }, function () {
                        NavItems.errmsg = 'Error gettingUser Data';
                    });
            };
            if ($routeParams.netid) {
                $scope.netid = $routeParams.netid;
                $scope.loadassessmentreport();
            }
        }])
        .controller('AssessmentUserListCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
        $scope.assessment_id = $routeParams.assessmentId;
        $scope.netid = '';
            var params;
            params = {
        ...
        .controller('AssessmentMUserListCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
            $scope.masteryassessment_id = $routeParams.masteryassessmentId;
            $scope.netid = '';
            var params;
             params = {
                    callback: 'JSON_CALLBACK',
                    methodtype: 'GET',
                    masteryassessment_id: $scope.masteryassessment_id
                };
                $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.muserlist + '?' + $.param(params))
                    .then(function(data) {
                        if (data) {
                            $scope.mastery_view = data.data.mastery_view;
                        } else {
                            NavItems.errmsg = 'Error getting assessments';
                        }
                    }, function() {
        ...
        .controller('AssessmentMUserLookCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
            $scope.user_type = Auth.getUserType();
            $scope.masteryassessment_id = $routeParams.masteryassessmentId;
            $scope.netid = '';
            $scope.loadmasteryassessmentreport = function () {
                var params;
                 params = {
                    masteryassessment_id: $scope.masteryassessment_id,
                    netid: $scope.netid,
                ...
                $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.muserlookup + '?' + $.param(params))
                    .then(function (data) {
                        if (data.data) {
                            $scope.mastery_view = data.data.mastery_view;
                        } else {
                            NavItems.errmsg = 'Error getting assessments List for user';
                        }
                    }, function () {
                        NavItems.errmsg = 'Error getting assessments list fot user';
                    });
            };
        }])
        .controller('AssessmentReportCtrl', ['$scope', '$routeParams', '$http', 'globalConfig', 'Auth', 'NavItems', function ($scope, $routeParams, $http, globalConfig, Auth, NavItems) {
            $scope.user_type = Auth.getUserType();
            $scope.assessment_id = $routeParams.assessmentId;
            var params = {assessment_id: $scope.assessment_id, callback: 'JSON_CALLBACK'};
            $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.assessmentreport + '?' + $.param(params))
                .then(function (data) {
                    if (data.data && data.data.result) {
                        $scope.result = data.data.result.list;
                        $scope.totalstudents=data.data.result.total_students;
                        $scope.totalanswered=data.data.result.students_answered;
                    } else {
                        NavItems.errmsg = 'Error getting assessment report';
                    }
                }, function () {
                    NavItems.errmsg = 'Error getting assessment report';
                });
        ...
        .controller('AssessmentMListCtrl', ['$scope', '$routeParams', '$http', '$sce', '$timeout', 'globalConfig', 'Auth', 'NavItems', function($scope, $routeParams, $http, $sce, $timeout, globalConfig, Auth, NavItems) {
            $scope.user_type = Auth.getUserType();
            ...
            $http.jsonp(globalConfig.service.elearning + globalConfig.service.assessment.listmastery + '?' + $.param(params))
                .then(function(data) {
                    if (data.data.masteries) {
                        $scope.masteries = data.data.masteries;
                    } else {
                        NavItems.errmsg = 'Error getting Mastery Assessment List';
                    }
                }, function() {
                    NavItems.errmsg = 'Error getting Mastery Assessment List';
                });
